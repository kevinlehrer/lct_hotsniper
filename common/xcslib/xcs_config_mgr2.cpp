...
xcs_config_mgr2::xcs_config_mgr2() 
{
	std::cout << "[XCS][ConfigManager] reading config file ..." << endl;
	char cwd[256];
   	if (getcwd(cwd, sizeof(cwd)) != NULL)
       	std::cout << "[XCS][ConfigManager] Current working dir: " << cwd << endl;
	
	//! save file extension;
	file_extension = std::string("config");

	//! open the configuration file
	string configFile = "../common/scheduler/policies/xcs." + file_extension;
...
void
xcs_config_mgr2::print_something()
{
	std::cout << "[XCS][ConfigManager] print something" << endl;
}
