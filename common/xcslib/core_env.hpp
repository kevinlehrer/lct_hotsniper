...
class core_env : public virtual environment_base
{
 public:
	string class_name() const { return string("core_env"); };
	string tag_name() const { return string("environment::core"); };

	//! Constructor for the core class that passes an object to access simulators measurments
	/*!
	 *  This is the first constructor that must be used. Otherwise an error is returned.
	 */
	core_env(xcs_config_mgr2&, const PerformanceCounters*);
		
	//! Constructor for the multiplexer class that read the class parameters through the configuration manager
	/*!
	 *  This is the first constructor that must be used. Otherwise an error is returned.
	 */
	core_env(xcs_config_mgr2&);
 ...
 private:
	...
	//! reference to performance counters
	const PerformanceCounters* measurements;
